...
public class FuncionarioView {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Object> getFuncId(@PathVariable("id") int cod) {
    ...
    @GetMapping("/buscar")
    public ResponseEntity<Object> buscarPorEmail(@RequestParam("email") String email) {
        Map<String, Object> json = funcionarioController.buscarFuncionarioPorEmail(email);

        if (json != null) {
            return ResponseEntity.ok(json);
        } else {
            return ResponseEntity.status(404).body(new Mensagem("Funcionário não encontrado."));
        }
    }
